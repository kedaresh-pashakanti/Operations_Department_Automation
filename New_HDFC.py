...
MPR_RULES = [
    ...
    (["NEFT CR-ICIC0000105-WORLDLINE EPAYMENTS INDIA PRIVATE LIMITED", "ICIC0099999-WORLDLINE EPAYMENTS",], "12-WorldLine NB"),
...
def normalize_lookup_key(value):
    """Normalize reference-style keys for consistent matching across files."""
...
def normalize_column_key(value):
    """Normalize column names to compare regardless of spaces/punctuation."""
    return re.sub(r"[^A-Z0-9]+", "", normalize_text(value))


def build_irctc_refund_sp_map(refund_df):
    """
    Build a lookup of IRCTC refund ARN -> SP Identifier/MID.

    Expected columns:
    - ARN
    - udf1

    If udf1 contains 'irctcmpp@hdfcbank', assign M00015.
    """
    mapping = {}
    ...
    if not arn_col or not udf1_col:
        return mapping

    for _, row in refund_df.iterrows():
        arn = normalize_lookup_key(row.get(arn_col, ""))
        udf1 = safe_str(row.get(udf1_col, "")).strip()
        if arn and "IRCTCMPP@HDFCBANK" in normalize_text(udf1):
            mapping[arn] = "M00015"

    return mapping


def build_arn_merchant_refund_sp_map(refund_df):
    """
    Build a lookup of refund ARN/ArnNo -> Merchant_Id.
    Works for PA Refund and PG Refund files.
    """
    mapping = {}
...
def build_irctc_refund_sp_map_from_paths(refund_paths):
    merged = {}
    if not refund_paths:
        return merged
    for path in refund_paths:
        ...
        merged.update(build_irctc_refund_sp_map(refund_df))
    return merged


def build_arn_merchant_refund_sp_map_from_paths(refund_paths):
    merged = {}
    if not refund_paths:
        return merged
    for path in refund_paths:
        if not path or not os.path.exists(path):
            continue
        refund_df = read_input_file(path)
        merged.update(build_arn_merchant_refund_sp_map(refund_df))
    return merged


def build_special_refund_sp_map_from_paths(irctc_paths=None, pa_paths=None, pg_paths=None):
    """
    Build one combined map for refund SP Identifier/MID resolution.
    Priority: IRCTC -> PA -> PG (later files overwrite earlier ones if duplicate).
    """
    merged = {}
    merged.update(build_irctc_refund_sp_map_from_paths(irctc_paths))
    merged.update(build_arn_merchant_refund_sp_map_from_paths(pa_paths))
    merged.update(build_arn_merchant_refund_sp_map_from_paths(pg_paths))
    return merged


def resolve_special_refund_sp_from_reference(reference_no, special_refund_sp_map):
    """
    ...
    resolved = special_refund_sp_map.get(ref)
    if resolved:
        return resolved

    for key, value in special_refund_sp_map.items():
        key_norm = normalize_lookup_key(key)
        if ref == key_norm or ref in key_norm or key_norm in ref:
            return value
...
def apply_tag_logic(df, refund_rrn_map=None, special_refund_sp_map=None):
    ...
    # New requirement: if SP is still blank for Refund rows, resolve from
...
def build_processed_workbook_and_df(input_file_path, template_path, refund_file_path=None, refund_rrn_map=None, special_refund_sp_map=None):
    ...
    df = apply_tag_logic(df, refund_rrn_map=refund_rrn_map, special_refund_sp_map=special_refund_sp_map)
...
def process_workbook(input_file_path, template_path, output_file_path, refund_file_path=None, refund_rrn_map=None, special_refund_sp_map=None):
    wb, _ = build_processed_workbook_and_df(input_file_path, template_path, refund_file_path=refund_file_path, refund_rrn_map=refund_rrn_map, special_refund_sp_map=special_refund_sp_map)
    wb.save(output_file_path)
    return output_file_path


def process_workbook_to_bytes(input_file_path, template_path, refund_file_path=None, refund_rrn_map=None, special_refund_sp_map=None):
    wb, df = build_processed_workbook_and_df(input_file_path, template_path, refund_file_path=refund_file_path, refund_rrn_map=refund_rrn_map, special_refund_sp_map=special_refund_sp_map)
    output = io.BytesIO()
    wb.save(output)
    output.seek(0)
    return output, df
...
def run_streamlit_app():
    ...
    irctc_uploads = st.file_uploader(
        "Upload IRCTC Refund Files",
        type=["xlsx", "xlsm", "xls", "csv"],
        accept_multiple_files=True,
    )
    pa_uploads = st.file_uploader(
        "Upload PA Refund Files",
        type=["xlsx", "xlsm", "xls", "csv"],
        accept_multiple_files=True,
    )
    pg_uploads = st.file_uploader(
        "Upload PG Refund Files",
        type=["xlsx", "xlsm", "xls", "csv"],
        accept_multiple_files=True,
    )
    ...
    if irctc_uploads:
        st.info(f"{len(irctc_uploads)} IRCTC refund file(s) uploaded successfully")

    if pa_uploads:
        st.info(f"{len(pa_uploads)} PA refund file(s) uploaded successfully")

    if pg_uploads:
        st.info(f"{len(pg_uploads)} PG refund file(s) uploaded successfully")
        ...
        if st.button("Process Files"):
            ...
            try:
                with st.spinner("Preparing files..."):
                    ...
                    if irctc_uploads:
                        for upload in irctc_uploads:
                            suffix = os.path.splitext(upload.name)[1] or ".xlsx"
                            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp_ref:
                                tmp_ref.write(upload.getbuffer())
                                temp_irctc_paths.append(tmp_ref.name)

                    if pa_uploads:
                        for upload in pa_uploads:
                            suffix = os.path.splitext(upload.name)[1] or ".xlsx"
                            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp_ref:
                                tmp_ref.write(upload.getbuffer())
                                temp_pa_paths.append(tmp_ref.name)

                    if pg_uploads:
                        for upload in pg_uploads:
                            suffix = os.path.splitext(upload.name)[1] or ".xlsx"
                            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp_ref:
                                tmp_ref.write(upload.getbuffer())
                                temp_pg_paths.append(tmp_ref.name)

                    special_refund_sp_map = build_special_refund_sp_map_from_paths(
                        irctc_paths=temp_irctc_paths,
                        pa_paths=temp_pa_paths,
                        pg_paths=temp_pg_paths,
                    )
                    ...
                    try:
                        with st.spinner(f"Processing {input_upload.name}..."):
                            ...
                            output_bytes, processed_df = process_workbook_to_bytes(
                                temp_input_path,
                                temp_template_path,
                                special_refund_sp_map=special_refund_sp_map,
                            )
                ...
                if temp_template_path and os.path.exists(temp_template_path):
                    try:
                        ...
                        pass
                for p in temp_irctc_paths + temp_pa_paths + temp_pg_paths:
