...
# =========================
    ...
    # KOTAK JUSPAY UPI
    # =========================
    elif key == "KotakJusPay_upi":
        try:
            # Keep Order ID as TEXT
            ...
            # Convert financial columns to REAL NUMBERS
            for amount_col in [
                "Transaction Amount",
                "Fee",
                "Tax",
...
vendors = [
    ...
    {"key": "KotakJusPay_upi", "name": "KotakJusPay_UPI", "candidates": ["Transaction Amount"], "mode": "multiple"},
]
...
OUTPUT_SHEET_SPECS = [
    ...
    ("15kotakjuspayupi", "KotakJusPay_UPI", None),
...
def build_sql_style_report(excel_data, selected_mpr_date):
    ...
    # 20-KotakJusPay UPI
    df = excel_data.get("KotakJusPay_UPI")
    ...
    if isinstance(df, pd.DataFrame) and not df.empty:
        ...
        # ---------------------------------------------------------
        # Process only when mandatory columns are available
        # ---------------------------------------------------------
        if (
            txn
            and gross_col
            ...
            if not dfx.empty:
                ...
                #
                # Anything else
                #   → 20-KotakJusPay UPI - <Type>
                #
                # Example:
                # Type = REFUND
                # SP_Name = 20-KotakJusPay UPI - REFUND
                #
                # Type = CHARGEBACK
                # SP_Name = 20-KotakJusPay UPI - CHARGEBACK
                # -------------------------------------------------
                type_values = (
                    dfx[type_col]
                    .fillna("")
...
# =========================================================
# PHASE 2 - HDFC ESCROW WORKBOOK INTEGRATION
# =========================================================

HDFC_ESCROW_SHEET = "CR - HDFC ESCROW"
MPR_EXTRACT_SHEET = "MPR Extract "


def clean_lookup_id(value):
    """
    Clean IDs for safe matching between SQL report and MID file.
    """
    if value is None:
        return ""
...
def read_mid_ids_file(mid_file_bytes):
    """
    Read MID IDs Excel file.
    ...
    mid_df = pd.read_excel(
        BytesIO(mid_file_bytes),
        dtype=str
    )

    mid_df.columns = [
        str(c).strip()
        for c in mid_df.columns
    ]
    ...
    for col in mid_df.columns:
        ...
        if normalized == "1paytransactionid":
            txn_col = col

        elif normalized == "merchantid":
            merchant_col = col

    if not txn_col or not merchant_col:

        raise ValueError(
            "MID file must contain "
            "'1Pay Transaction ID' and 'Merchant ID' columns."
        )
    ...
    for _, row in mid_df.iterrows():
        ...
        txn_id = clean_lookup_id(
            row.get(txn_col, "")
        )

        merchant_id = str(
            row.get(merchant_col, "")
            or ""
        ).strip()

        if txn_id:
            mid_mapping[txn_id] = merchant_id

    return mid_mapping


def prepare_hdfc_statement(statement_file_bytes):
    """
    Read statement CSV and run the EXISTING
    HDFC ESCROW processing rules.
    """

    statement_df = pd.read_csv(
        BytesIO(statement_file_bytes),
        dtype=str
    )

    statement_df.columns = [
        str(c).strip()
        for c in statement_df.columns
    ]

    # Use the existing HDFC column preparation.
    statement_df = ensure_standard_logic_columns(
        statement_df
    )

    # Ensure existing HDFC logic columns exist.
    for col in [
        "Tranaction Tag",
        "SP Identifier/MID",
        "Split Refunds",
        ...
        if col not in statement_df.columns:
            statement_df[col] = ""

    # Use the EXISTING HDFC rule engine.
    statement_df = apply_tag_logic(
        statement_df,
        refund_rrn_map={},
...
def write_statement_to_hdfc_sheet(
    ws,
    statement_df
):
    """
    Write the processed bank statement into:
...
def write_sql_to_mpr_extract(
    ws,
    sql_report_df,
    mid_mapping
):
    """
    Write existing SQL report into:

        MPR Extract

    Template:

        Row 1 = existing formula
        Row 2 = headers
        Row 3 onward = data

    Existing formula in row 1 is NOT changed.
    """

    # ---------------------------------------------------------
    # Existing workbook currently has blank H/I headers.
    # Add the two new SQL columns.
    # ---------------------------------------------------------
    ...
    for row_idx, (_, row) in enumerate(
        sql_report_df.iterrows(),
        start=3
    ):

        transaction_id = clean_lookup_id(
            row.get(
                "1Pay_Transaction_ID",
                ""
            )
...
def build_hdfc_escrow_final_workbook(
    workbook_bytes,
    mid_ids_bytes,
    statement_bytes,
    sql_report_df
    ...
    """
    Final Phase-2 workbook.

    Existing vendor processing is already complete.

    This function only:
        1. Reads MID file
        2. Processes statement using existing HDFC rules
        3. Loads uploaded workbook
        4. Writes SQL -> MPR Extract
        5. Writes statement -> CR - HDFC ESCROW
        6. Preserves existing workbook formulas
    """

    if (
        sql_report_df is None
        or not isinstance(
            sql_report_df,
            pd.DataFrame
        )
    ...
    mid_mapping = read_mid_ids_file(
        mid_ids_bytes
    )

    # ---------------------------------------------------------
    # 2. Process bank statement using EXISTING HDFC logic
    # ---------------------------------------------------------

    statement_df = prepare_hdfc_statement(
        statement_bytes
    )

    # ---------------------------------------------------------
    # 3. Open uploaded workbook.
    #
    # data_only=False is IMPORTANT because existing formulas
    # must remain formulas.
    # ---------------------------------------------------------
    ...
    if MPR_EXTRACT_SHEET not in wb.sheetnames:

        raise ValueError(
            f"Workbook sheet not found: "
            f"{MPR_EXTRACT_SHEET!r}"
        )

    if HDFC_ESCROW_SHEET not in wb.sheetnames:

        raise ValueError(
            f"Workbook sheet not found: "
            f"{HDFC_ESCROW_SHEET!r}"
        )
    ...
    write_sql_to_mpr_extract(
        wb[MPR_EXTRACT_SHEET],
        sql_report_df,
        mid_mapping
    )

    # ---------------------------------------------------------
    # 6. Statement -> CR - HDFC ESCROW
    # ---------------------------------------------------------

    write_statement_to_hdfc_sheet(
        wb[HDFC_ESCROW_SHEET],
        statement_df
    )

    # ---------------------------------------------------------
    # 7. Force Excel recalculation.
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 8. Save updated workbook to memory.
    # ---------------------------------------------------------
...
workbook_upload = st.file_uploader(
    ...
    type=["xlsx", "xlsm"],
    key="hdfc_workbook_upload",
)

mid_ids_upload = st.file_uploader(
    "2. Upload MID IDs File",
    type=["xlsx", "xlsm", "xls"],
    key="hdfc_mid_ids_upload",
)

statement_upload = st.file_uploader(
    "3. Upload Statement File",
    type=["csv"],
    key="hdfc_statement_upload",
)
