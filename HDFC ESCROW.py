...
RULES_STORE_FILE = "extra_sp_mid_rules.json"


def get_rules_store_path():
    base_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else os.getcwd()
    return os.path.join(base_dir, RULES_STORE_FILE)


def normalize_tid_mid_rules(rules):
    """Return a clean list of (TID, MID) tuples."""
    cleaned = []
    seen = set()
    ...
    for item in rules or []:
        if not item or len(item) < 2:
            continue
        tid = safe_str(item[0]).strip()
        mid = safe_str(item[1]).strip()
        ...
        if key in seen:
            continue
        seen.add(key)
        cleaned.append((tid, mid))

    return cleaned


def load_extra_sp_mid_rules():
    """Load TID/MID rules from disk; fall back to defaults if the file is missing."""
    path = get_rules_store_path()
        ...
        try:
            ...
            for item in data:
                if isinstance(item, dict):
                    tid = safe_str(item.get("TID", "")).strip()
                    mid = safe_str(item.get("MID", "")).strip()
                    if tid and mid:
                        rules.append((tid, mid))
                    ...
                    if tid and mid:
                        rules.append((tid, mid))
            if rules:
                return normalize_tid_mid_rules(rules)
        except Exception:
            pass

    return normalize_tid_mid_rules(DEFAULT_EXTRA_SP_MID_RULES)


def save_extra_sp_mid_rules(rules):
    """Persist TID/MID rules to disk."""
    path = get_rules_store_path()
    payload = [{"TID": tid, "MID": mid} for tid, mid in normalize_tid_mid_rules(rules)]
...
EXTRA_SP_MID_RULES = load_extra_sp_mid_rules()
...
def get_current_extra_sp_mid_rules():
    """Return the currently active TID/MID rules list."""
    if "extra_sp_mid_rules" in globals():
        return normalize_tid_mid_rules(EXTRA_SP_MID_RULES)
    return normalize_tid_mid_rules(DEFAULT_EXTRA_SP_MID_RULES)


def sync_extra_sp_mid_rules_from_session(rules):
    """Update the global rule list used by the processing functions."""
    global EXTRA_SP_MID_RULES
    EXTRA_SP_MID_RULES = normalize_tid_mid_rules(rules)
    save_extra_sp_mid_rules(EXTRA_SP_MID_RULES)
...
def run_streamlit_app():
    ...
    if "extra_sp_mid_rules" not in st.session_state:
        st.session_state.extra_sp_mid_rules = load_extra_sp_mid_rules()

    with st.expander("TID / MID Mapping Rules", expanded=True):
        st.write("Current TID → MID rules")
        rules_df = pd.DataFrame(st.session_state.extra_sp_mid_rules, columns=["TID", "MID"])
        st.dataframe(rules_df)

        col1, col2 = st.columns(2)
        with col1:
            new_tid = st.text_input("Add TID", key="new_tid_input")
        with col2:
            new_mid = st.text_input("Add MID", key="new_mid_input")

        add_btn = st.button("Add TID and MID")
        continue_btn = st.button("Continue with Existing TID MID")

        if add_btn:
            tid = safe_str(st.session_state.get("new_tid_input", "")).strip()
            mid = safe_str(st.session_state.get("new_mid_input", "")).strip()

            if not tid or not mid:
                st.error("Both TID and MID are required.")
            else:
                updated = list(st.session_state.extra_sp_mid_rules)
                replaced = False
                for i, (existing_tid, existing_mid) in enumerate(updated):
                    if existing_tid == tid:
                        updated[i] = (tid, mid)
                        replaced = True
                        break
                ...
                if not replaced:
                    updated.append((tid, mid))

                st.session_state.extra_sp_mid_rules = normalize_tid_mid_rules(updated)
                sync_extra_sp_mid_rules_from_session(st.session_state.extra_sp_mid_rules)
                st.success(f"TID {tid} and MID {mid} added successfully.")

        if continue_btn:
            sync_extra_sp_mid_rules_from_session(st.session_state.extra_sp_mid_rules)
            st.success("Continuing with existing TID/MID rules.")
